...
X_encoded = pd.get_dummies(X, columns=['cp', 'slope', 'restecg'], drop_first=False)
...
scores_11 = cross_val_score(knn_11, X_scaled, y, cv=5, scoring='accuracy')
...
# First, create the example data with the original features
example1_original = pd.DataFrame([[50, 1, 3, 222, 0, 2]], 
                                 columns=['age', 'sex', 'cp', 'thalach', 'slope', 'restecg'])

# Apply one-hot encoding
example1_encoded = pd.get_dummies(example1_original, columns=['cp', 'slope', 'restecg'], drop_first=False)

# Make sure it has the same columns as the training data
# Get the column names from X_encoded (before scaling)
X_encoded_columns = X_encoded.columns.tolist()

# Reindex example1_encoded to match the columns in X_encoded
for col in X_encoded_columns:
    if col not in example1_encoded.columns:
        example1_encoded[col] = 0

example1_encoded = example1_encoded[X_encoded_columns]

# Scale the example
example1_scaled = scaler.transform(example1_encoded)

# Make prediction
prediction1 = knn.predict(example1_scaled)
...
# First, create the example data with the original features
example2_original = pd.DataFrame([[63, 0, 1, 100, 2, 0]], 
                                 columns=['age', 'sex', 'cp', 'thalach', 'slope', 'restecg'])

# Apply one-hot encoding
example2_encoded = pd.get_dummies(example2_original, columns=['cp', 'slope', 'restecg'], drop_first=False)

# Reindex example2_encoded to match the columns in X_encoded
for col in X_encoded_columns:
    if col not in example2_encoded.columns:
        example2_encoded[col] = 0

example2_encoded = example2_encoded[X_encoded_columns]

# Scale the example
example2_scaled = scaler.transform(example2_encoded)

# Make prediction
prediction2 = knn.predict(example2_scaled)
